...
- 커스텀 모델 내보내기 스크립트 `tool/export_custom_model.py` 추가. README에 있던 명령(`yolo export format=tflite int8=True`)으로 만든 Android 파일은 입력 레이아웃(NCHW)과 텐서 이름이 플러그인과 맞지 않아 탐지가 0건이었다.
