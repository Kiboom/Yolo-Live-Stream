...
- 0.12.0 학습 스크립트로 만든 강아지 포즈 모델을 Android에서 불러오지 못하던 버그 수정. 플러그인은 입력 `images`, 출력 `Identity`라는 이름으로 텐서를 찾는데, 스크립트가 만든 `.tflite`는 이름이 달랐다. 이제 학습 스크립트가 이 이름으로 내보내고, 내보낸 뒤 이름과 모양을 검사한다. 이미 만든 `.tflite`는 `--weights`로 다시 내보내야 한다.
- example 앱의 `dog_pose.tflite`도 같은 가중치로 다시 내보내 텐서 이름을 맞췄다.
- 포즈 모델을 불러오지 못해도 사람과 강아지 탐지는 계속한다. `onError`로 `포즈 모델 로드 실패: ...`를 한 번 알리고, 위험도는 거리와 으르렁 점수만으로 판정한다. 0.12.0에서는 `YOLO 모델 로드 실패: ...`를 알리고 YOLO 분석 전체가 시작되지 않았다.
- 커스텀 모델 내보내기 스크립트 `tool/export_custom_model.py` 추가. Ultralytics 8.4.83 이상에서 README에 있던 명령(`yolo export format=tflite int8=True`)으로 만든 Android 파일은 입력 레이아웃(NCHW)과 텐서 이름이 플러그인과 맞지 않아 탐지가 0건이었다. 8.4.83부터 `format=tflite`가 LiteRT 내보내기로 바뀌었기 때문이다.
- 새 스크립트는 Android용 `.tflite`(NHWC 입력, 동적 int8, end-to-end)와 iOS용 `.mlpackage.zip`(int8, end-to-end)을 만든다. 내보낸 뒤 모양과 이름을 검사하고, 플러그인이 읽을 수 없으면 실패한다. `tool/train_dog_pose.py`도 같은 내보내기 코드를 쓴다.
- README 커스텀 모델 안내를 새 스크립트 기준으로 바꿨다.
